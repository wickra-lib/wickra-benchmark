//! Deterministic JSON canonicalization — the single source of a case hash's
//! stability across all ten language bindings.
//!
//! This is byte-for-byte the same canonicalization `wickra-proof` uses, on
//! purpose: a case's `expected_hash` over the recomputed report equals the proof
//! hash the same report would produce, so a benchmark result and a proof are
//! cross-checkable. (Until `wickra-proof` publishes a library crate this lives
//! here; it is a verbatim copy of that definition and is meant to be swapped for
//! the shared dependency later.) The rules (each normative):
//!
...
/// The canonical blake3 hash of a serializable value (typically a
/// `BacktestReport`): `hash(&canonicalize(value)?)`. This is exactly how a
/// case's `expected_hash` is defined, so a recomputed report that matches the
/// expectation reproduces the frozen hash.
