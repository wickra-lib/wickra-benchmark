...
/**
 * A benchmark runner driven by JSON commands, over the Wickra C ABI (FFM/Panama).
 * Create one, drive it with command JSON ({@code run_case}, {@code run_suite},
 * {@code list_cases}, {@code version}) and read back the response JSON — the same
...
public final class Benchmark implements AutoCloseable {
    ...
    public Benchmark() {
        try {
            ...
            if (created.address() == 0) {
                throw new IllegalStateException("wickra-benchmark: failed to create a benchmark");
            }
            this.handle = created;
     ...
     * read into a caller-owned buffer. Domain errors (a bad case, an unknown
    ...
    public String command(String cmdJson) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            if (len < 0) {
                throw new IllegalStateException("wickra-benchmark: command failed (code " + len + ")");
            }
    ...
    /** Free the native benchmark handle. */
    @Override
    public void close() {
        if (handle != null) {
            try {
