//! The wickra-benchmark C ABI — the hub every C-capable language links against.
//!
//! The surface is deliberately tiny and JSON-shaped, exactly like
//! `Benchmark::command_json`: create a benchmark handle, drive it with command
//! JSONs (`run_case`, `run_suite`, `list_cases`, `version`) and read back
//! response JSONs, then free the handle. No result type crosses the boundary by
...
//!
//! 1. Call [`wickra_benchmark_command`] with `out = NULL`, `cap = 0` to learn
...
//! for unusable arguments ([`WICKRA_BENCHMARK_ERR_NULL`],
//! [`WICKRA_BENCHMARK_ERR_UTF8`]) and caught panics
//! ([`WICKRA_BENCHMARK_ERR_PANIC`]); a non-negative return is always the
//! response length. Domain errors (a bad case, an unknown command) are *not*
...
pub const WICKRA_BENCHMARK_ERR_UTF8: i32 = -2;
/// A panic was caught at the FFI boundary.
pub const WICKRA_BENCHMARK_ERR_PANIC: i32 = -3;
...
pub struct WickraBenchmark(Benchmark);
...
#[no_mangle]
pub extern "C" fn wickra_benchmark_new() -> *mut WickraBenchmark {
    Box::into_raw(Box::new(WickraBenchmark(Benchmark::new())))
}
...
#[no_mangle]
pub unsafe extern "C" fn wickra_benchmark_free(handle: *mut WickraBenchmark) {
    if !handle.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn wickra_benchmark_command(
    handle: *mut WickraBenchmark,
    cmd_json: *const c_char,
    out: *mut c_char,
    cap: usize,
) -> i32 {
    if handle.is_null() || cmd_json.is_null() {
        return WICKRA_BENCHMARK_ERR_NULL;
    }
    let Some(cmd) = (unsafe { opt_str(cmd_json) }) else {
        return WICKRA_BENCHMARK_ERR_UTF8;
    };
    let benchmark = unsafe { &(*handle).0 };
    let response = match catch_unwind(AssertUnwindSafe(|| benchmark.command_json(cmd))) {
        ...
        Err(_) => return WICKRA_BENCHMARK_ERR_PANIC,
    };
...
#[no_mangle]
pub extern "C" fn wickra_benchmark_version() -> *const c_char {
...
mod tests {
    ...
    fn new_command_free_round_trip() {
        ...
        unsafe { wickra_benchmark_free(handle) };
    }
    ...
    fn bad_command_reports_error_in_band() {
        ...
        let len = unsafe { wickra_benchmark_command(handle, bad.as_ptr(), ptr::null_mut(), 0) };
    ...
    fn null_guards_on_command() {
        ...
        // Null handle.
        let code =
            unsafe { wickra_benchmark_command(ptr::null_mut(), cmd.as_ptr(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_BENCHMARK_ERR_NULL);
        // Null command with a valid handle.
        let handle = wickra_benchmark_new();
        let code = unsafe { wickra_benchmark_command(handle, ptr::null(), ptr::null_mut(), 0) };
        assert_eq!(code, WICKRA_BENCHMARK_ERR_NULL);
        unsafe { wickra_benchmark_free(handle) };
    }
    ...
    fn free_null_is_a_noop() {
        unsafe { wickra_benchmark_free(ptr::null_mut()) };
    }

    #[test]
    fn version_is_nul_terminated() {
        let p = wickra_benchmark_version();
